        ...
        <div class="w-full">
          ...
          <div class="space-y-5 md:space-y-8">
            <form action="{{ route('search-history') }}" method="GET" style="float:right;">
              <input type="text" name="search" value="{{ $search }}" placeholder="Cari Nama..." style="background-color:rgb(223, 223, 223); padding:10px; border-radius:10px;">
            ...
            <table style="text-align:center; width:100%;">
              ...
              @foreach ($listHKamar as $key => $header)
                <tr style="background-color: rgb(223, 223, 223); border-bottom: 10px solid #ffffff;">
                  ...
                  @php
                    if($header->total == 0){
                      echo '<td>Rp '.number_format($header->harga , 0, ',', '.').'</td>';
                    }
                    else{
                      echo '<td>Rp '.number_format($header->total , 0, ',', '.').'</td>';
                    }
                  @endphp
                  <td>
                    @if($header->status == 1)
                  ...
                  <td style="padding-top:1%; padding-bottom:1%;">
                      @php
                        if ($getD != null) {
                          ...
                          echo '
                            <a class="py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none" href="/user/history/galon/'.$header->h_kamar_id.'">
                              Lihat Detail
                              <svg class="flex-shrink-0 w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                            </a>
                          ';
